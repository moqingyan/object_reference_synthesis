    ...
    for i, c_i in enumerate(interpreter.clauses):
        ...
        for j, c_j in enumerate(interpreter.clauses):
            ...
            for h, c_h in enumerate(interpreter.clauses):
                ...
                for k, c_k in enumerate(interpreter.clauses):
                    state_ijhk = interpreter.step(state_ijh, c_k, is_uncertain)
                    for l, c_l in enumerate(interpreter.clauses):
                        state_ijhkl = interpreter.step(state_ijhk, c_l, is_uncertain)
                        if interpreter.is_state_correct(target_object, state_ijhkl, is_uncertain):
                            return Program([c_i, c_j, c_h, c_k, c_l])
...
if __name__ == '__main__':
    ...
    # Step 2: Grounding
    unary = {'large': [0, 1, 2, 3, 4, 5], 'small': [6], 'red': [0, 1, 2, 3, 4, 6], 'blue': [5], 'metal':  [0, 1, 2, 3, 5, 6], 'rubber': [4], 'sphere':  [0, 1, 2, 4, 5, 6], 'cube': [3]}
    binary = {'left': [], 'front': []}
    ...
    unary_uncertain = {'large': [], 'small': [], 'red': [], 'blue': [], 'metal':  [], 'rubber': [], 'sphere':  [], 'cube': []}
    ...
    # Step 7: Synthesis
    t = time.time()
    program_synth = synthesize(interpreter, target_object, interpreter.reset(), False, 6)
