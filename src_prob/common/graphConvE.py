...
class GraphConvE(MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, aggr='add', bias=True,
                 **kwargs):
        super(GraphConvE, self).__init__(aggr=aggr, **kwargs)
    ...
    def message(self, h_j, edge_weight):
        assert(edge_weight.shape[1] == self.out_channels)
        return h_j if edge_weight is None else edge_weight.view(-1, self.out_channels) + h_j
