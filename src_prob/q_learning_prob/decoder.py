...
class ClauseDecoder(nn.Module):
    ...
    def forward(self, graph_embeddings, state):
        ...
        x = []
        # for clause_idx in clauses_idxes:
        #     clause_info = torch.cat(
        #         (local_embedding[clause_idx], global_embedding
        #         )).view(-1)
        #     if len(clause_idx) == 2:
        #         x.append(self.common_layer(self.binary_op_layer(clause_info)))
        #     elif len(clause_idx) == 3:
        #         x.append(self.common_layer(self.ternary_op_layer(clause_info)))
        #     else:
        #         raise Exception("clause index dimension incorrect")
